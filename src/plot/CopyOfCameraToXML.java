...
public class CopyOfCameraToXML {
	...
	private Logger logger = Logger.getLogger(CopyOfCameraToXML.class.getName());
	...
	public Document CreateCamera(OWLModel owlModel, String maName, Document doc) {
		...
		if (listall.size() == 0)
			...
			if (bgtype == 3) // 室外场景
				doc = NewCameraPlan(owlModel, "newCameraA", doc, listall, 1, this.maFrame);
			else { // 室内场景
				...
				SpaceCameraPlan(listall, spacelist, targetSpaceList, owlModel, doc, startFrame, endFrame);
			}
		}
		logger.info("===摄像机程序结束===");
		return doc;
	...
	public Document printRule(Document doc, String cameraName, int isStart, String shotPlan,
		...
		String targetid = "";
		String targetname = "";
		for (int i = 0; i < targetIDs.size()&&i < targets.size(); i++) {
	...
	/*
	 * 获取目标类的ID组 需要从doc中获得 id
	 * 
	 * @param list
	 * 
	 * @return
	 */
	public ArrayList<String> getID(ADLRule list, Document doc) {
		ArrayList<String> ids = new ArrayList<String>();
		ArrayList<String> modelsname = list.getTarget();

		for (int i = 0; i < modelsname.size(); i++) {// 知识库中取出一个镜头的多个目标
			String modelnameOWL = modelsname.get(i);
			Element rootName = (Element) doc.getRootElement();
			Element name = rootName.element("maName");
			...
			Iterator<Element> it = name.elementIterator(); it.hasNext();) {
				...
				if (null != rule.attributeValue("ruleType") && rule.attributeValue("ruleType").equals("addToMa")) {
					...
					if (modelnameOWL.equals(modelnameADL)) {
						ids.add(modelidADL);
					}
				}
			}
	 ...
	 * @return
	 */
	public Document FullShot(Document doc, String cameraName, ArrayList<SceneSpace> list, int startframe, int endframe,
			int startall, int endall) {
		try {
			ArrayList<ADLRule> shots = getADLRulesFromOWL();

			/**
			 * 若规则推理出合适的HLCP镜头，则走规则；否则继续执行之前的随机
			 */
			if (shots != null && shots.size() > 0) {
				for (int i = 0; i < shots.size(); i++) {// 目前是一个手法里的镜头们均分帧数
					ADLRule shot = shots.get(i);
					String shotplan = shot.getShotPlan();
					String cameraNameByOWL = shot.getShotName();
					ArrayList<String> targets = shot.getTarget();
					...
					if (shotplan.equals("Fix")) {
						doc = printRule(doc, cameraNameByOWL, 1, "Fix", getID(shot, doc), targets, startPitch,
								endPitch, startRotate, endRotate, startSceneType, endSceneType, shot.getStartframe(),
								shot.getEndframe(), startall, endall);
					} else if (shotplan.equals("Push")) {
						doc = printRule(doc, cameraNameByOWL, 1, "Push", getID(shot, doc), targets, startPitch,
								endPitch, startRotate, endRotate, startSceneType, endSceneType, shot.getStartframe(),
								shot.getEndframe(), startall, endall);
					} else if (shotplan.equals("Pull")) {
						doc = printRule(doc, cameraNameByOWL, 1, "Pull", getID(shot, doc), targets, startPitch,
								endPitch, startRotate, endRotate, startSceneType, endSceneType, shot.getStartframe(),
								shot.getEndframe(), startall, endall);
					} else if (shotplan.equals("Rotate")) {
						doc = printRule(doc, cameraNameByOWL, 1, "Rotate", getID(shot, doc), targets, startPitch,
								endPitch, startRotate, endRotate, startSceneType, endSceneType, shot.getStartframe(),
								shot.getEndframe(), startall, endall);
					} else if (shotplan.equals("Follow")) {
						doc = printRule(doc, cameraNameByOWL, 1, "Follow", getID(shot, doc), targets, startPitch,
								endPitch, startRotate, endRotate, startSceneType, endSceneType, shot.getStartframe(),
								shot.getEndframe(), startall, endall);
					}
	...
	public Document NewCameraPlan(OWLModel owlModel, String cameraName, Document doc, ArrayList<SceneSpace> listAll,
			int startFrame, int endFrame) {
		...
		ArrayList<String> spaceList = getSpaceList(listAll);
		ArrayList<String> targetSpaceList = getSpaceList(targetIDs);// 目标拍摄物的可用空间
		int targetSpaceSize = targetSpaceList.size();// 目标拍摄物可用空间个数

		if (targetIDSize > 0) {
			int start = startFrame;
			int end = endFrame;
			int add = (end - start) / targetSpaceSize;// 按目标拍摄物的可用空间个数 均分摄像机帧数
			start = start - 1;
			end = start;// 初始为0

			for (int i = 0; i < targetSpaceSize; i++) {// 对于每个应用的可用空间
				start = end + 1;
				end = start + add;
				setSpaceType(getSpaceClass(owlModel, spaceList.get(i)));
				// 设置类型，spacelist为传入模型的可用空间
				// targetspacesize为目标模型所在的可用空间
				doc = FullShot(doc, cameraName, getSpaceScene(targetIDs, targetSpaceList.get(i)), start, end, start,
						end);
				setSpaceType(0);
	...
	public void SpaceCameraPlan(ArrayList<SceneSpace> listall, ArrayList<String> spacelist,
		...
		case 0:
			switch (spacelist.size()) {
			case 0:
				doc = NewCameraPlan(owlModel, "newCameraA", doc, listall, 1, framenum);
				break;
			case 1:
				doc = NewCameraPlan(owlModel, "newCameraA", doc, getSpaceScene(listall, spacelist.get(0)), 1, framenum);
				break;
			default:
				doc = NewCameraPlan(owlModel, "newCameraA", doc, getSpaceScene(listall, spacelist.get(0)), startFrame,
						(endFrame - startFrame) / 2 + startFrame);
				doc = NewCameraPlan(owlModel, "newCameraB", doc, getSpaceScene(listall, spacelist.get(1)),
						(endFrame - startFrame) / 2 + startFrame + 1, endFrame);
			}
			break;
		case 1:
			doc = NewCameraPlan(owlModel, "newCameraA", doc, getMaxIndexID(listall), startFrame,
					(endFrame - startFrame) / 2 + startFrame);// 得分最大的，99%是istarget=1
			if (spacelist != null)
				doc = NewCameraPlan(owlModel, "newCameraB", doc,
						getSpaceScene(listall, spacelist.get(spacelist.size() - 1)), (endFrame - startFrame) / 2
								+ startFrame + 1, endFrame);
			break;
		case 2:
			doc = NewCameraPlan(owlModel, "newCameraA", doc, getSpaceScene(listall, targetSpacelist.get(0)),
					startFrame, (endFrame - startFrame) / 2 + startFrame);
			doc = NewCameraPlan(owlModel, "newCameraB", doc, getSpaceScene(listall, targetSpacelist.get(1)),
					(endFrame - startFrame) / 2 + startFrame + 1, endFrame);
			break;
		default:
			doc = NewCameraPlan(owlModel, "newCameraA", doc, getSpaceScene(listall, targetSpacelist.get(0)),
	...
	public ArrayList<ADLRule> getADLRulesFromOWL() {
		...
		try {
			...
			if (hlcpindiSelected != null) {
				...
				OWLObjectProperty ShotTargets = owlModel.getOWLObjectProperty(PREFIX_OWL + "ShotTargets");
				...
				int start = 0, end = 0;
				int remainder = this.maFrame;// 未分配的余下帧数
				for (Iterator<?> it = HLCPshots.iterator(); it.hasNext();) {
					...
					Collection<?> shotTargets = shot.getPropertyValues(ShotTargets);// shot实例的targets
																					// object
					ArrayList<String> TargetNameList = new ArrayList<String>();// shot实例的targets
																				// object
																				// 名称
					...
					try {
						startAndEnd = setStartEndTimeByRatio(time, start, end, remainder);
						start = startAndEnd[0];
						end = startAndEnd[1];
					...
					for (Iterator<?> target = shotTargets.iterator(); target.hasNext();) {
						OWLIndividual targetModel = (OWLIndividual) target.next();
						String targetInOWLShot = targetModel.getBrowserText();
						TargetNameList.add(targetInOWLShot);
					}
					...
					rule.setTarget(TargetNameList);// 每个镜头实例的target
				...
				if (remainder != 0) {
					...
					rule = ADLRules.get(ADLRules.size() - 1);
			...
			// Object ob=indi.getName();
			// 得到的是：http://www.owl-ontologies.com/MyCamera.owl#Coaxile_3_instance
	...
	public int[] setStartEndTimeByRatio(String ratio, int start, int end, int remainder) throws Exception {
		int[] ret = new int[3];
	...
	public static void main(String[] str) {
		CopyOfCameraToXML camera = new CopyOfCameraToXML();
	...
	class ADLRule {
		...
		public String getUsedModelID() {
			return usedModelID;
		}

		public void setUsedModelID(String usedModelID) {
		...
		public String toString() {
			String ret = "\"CameraName\":" + this.getShotName() + ";\"ShotPlan\":" + this.getShotPlan()
					+ ";\"target\":" + this.getTarget().toString() + ";\"StartPitch\":" + this.getStartPitch()
					+ ";\"EndPitch\":" + this.getEndPitch() + ";\"StartYaw\":" + this.getStartYaw() + ";\"EndYaw\":"
					+ this.getEndYaw() + ";\"StartShotType\":" + this.getStartShotType() + ";\"EndShotType:"
					+ this.getEndShotType() + ";\"Startframe\":" + this.getStartframe() + ";\"Endframe\":"
					+ this.getEndframe();
			return ret;
		}
